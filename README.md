...
A deep learning solution for brain tumor classification using MRI scans from the Kaggle dataset: https://www.kaggle.com/datasets/prathamgrover/brain-tumor-classification/data

## Introduction:
This project contains the code required to train a ResNet50 pretrained model to identify brain tumors based on the Kaggle dataset above.
...
Run ```pip install -r requirements.txt``` (or ```requirements-mac.txt``` for Mac)
...
Run ```./run_dashboard.sh``` (Mac/Linux) or ```streamlit run app.py```
